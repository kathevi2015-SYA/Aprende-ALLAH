...
const TriviaGame: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64 text-pink-500">
        <Loader2 className="w-10 h-10 animate-spin mr-2" /> Cargando preguntas...
      </div>
    );
  }
  ...
  if (currentQ >= questions.length) {
    return (
      <div className="max-w-lg mx-auto p-8 text-center">
        ...
        <button onClick={loadTrivia} className="bg-pink-500 text-white px-8 py-3 rounded-full font-bold hover:bg-pink-600">
          Jugar de Nuevo
        </button>
      </div>
    );
